...
#define PIXEL_OFF_THRESHOLD             10.0
...
bool MyRobot::cameraControl(void)
{
      ...
      valueToRotate += (previousGyro - m_gyroHorizontal->GetAngle()) * GYRO_CONVERSION;
      previousGyro = m_gyroHorizontal->GetAngle();

      // Check to see if the shooter is to be controled by the robot or by the operators
   if ((m_driverStation->GetDigitalIn(MINIBOT_PORT) == 0) || (IsAutonomous()
         == true))
      {
   ...
   if ((pixelOff < PIXEL_OFF_THRESHOLD) && (fabs(m_shooterWheel->GetOutputVoltage()
      - voltageToDriveShooter) < VOLTAGE_THRESHOLD))
   {
      readyToFire = true;
   }
   else 
   ...
   double &voltageToDriveShooter, double &valueToRotate, double &pixelOff)
{
   ...
   if (targetStatus == 0)
   {
      heightOfTriangle = sqrt(pow(results[TOP_TARGET][CENTER_OF_MASS_X_INDEX] -
                                ((results[LEFT_TARGET][CENTER_OF_MASS_X_INDEX] +
                                  results[RIGHT_TARGET][CENTER_OF_MASS_X_INDEX]) /
                                  2.0), 2.0) +
                              pow(results[TOP_TARGET][CENTER_OF_MASS_Y_INDEX] -
      ...
      {
         voltageToDriveShooter = 5.275914116 * exp(0.0286983122 * distanceToTarget);
      }
 ...
 * Sort the targets
 */
void MyRobot::sortTargetArray(double targets[NUMBER_OF_TARGETS][NUMBER_OF_TARGET_PARAMETERS])
{
